...
function normalizeErrorMessage(fallback: string, error: FirebaseLikeError | null): string {
...
async function readMigrationDocument(
  firestoreDb: Firestore,
  userId: string,
): Promise<MigrationDocument | null> {
  const snapshot = await getDoc(
    doc(firestoreDb, SCHEDULE_EVENT_MIGRATIONS_COLLECTION, userId),
  );
  return snapshot.exists()
    ? ({ ...snapshot.data(), userId } as MigrationDocument)
    : null;
}
...
function planEventSetOperation(plan: Parameters<typeof scheduleEventFromPlan>[0]): BatchOperation {
...
export function createFirebaseScheduleEventAuthority(
  ...
  return {
    async ensureMigrated(
      ...
      try {
        let state = await readMigrationDocument(firestoreDb, userId);
    ...
    async applyRecurringPlanMutation(userId, mutation) {
      try {
        ...
        mutation.actualUpserts.forEach((actual) => {
          batch.set(doc(firestoreDb, 'actuals', actual.id), stripUndefinedDeep(actual), {
            merge: true,
          });
        });
    ...
    async deletePlanWithDependents(mutation) {
      try {
        ...
        actuals.forEach((actual) => batch.delete(doc(firestoreDb, 'actuals', actual.id)));
        if (mutation.todo) {
          batch.set(
