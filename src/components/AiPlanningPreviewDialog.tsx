...
export function AiPlanningPreviewDialog({
  ...
  const signature = useMemo(
    () => blocks.map((block) => `${block.id}:${block.date}`).join('|'),
    [blocks],
  );
      ...
      >
        ...
        {mode === 'overview' ? (
          <>
            ...
            <p className="ai-planning-preview-hint">日付をタップすると、その日の予定だけを大きく表示します。</p>
                  ...
                  {groups.map((group) => (
                    ...
                    >
                      ...
                      {group.blocks.map((block) => (
                        <div
                          className={`ai-planning-draft-block ai-planning-preview-overview-block ${toneClass(block)}`}
                          key={block.id}
              ...
              <div className="ai-planning-preview-day-grid">
                ...
                <div className="ai-planning-preview-day-column-detail" style={{ height: `${detailTimelineHeight}px` }}>
                  ...
                  {selectedGroup?.blocks.map((block) => (
                    <div
                      className={`ai-planning-draft-block ai-planning-preview-detail-block ${toneClass(block)}`}
                      key={block.id}
