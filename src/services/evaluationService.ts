...
export function buildEvaluationSummary(
  ...
  if (!hasRecentPlans) {
    ...
    comment = '予定はありますが記録がまだありません。短時間でも1件記録すると評価が動きます。';
  } else if (dayPlans.length === 0) {
    comment = '今日は予定がありません。必要なら1件だけでも計画を入れて比較できる状態を作りましょう。';
  } else if (lowestScore === achievement) {
    comment =
      '記録が予定に届いていません。所要時間を少し短めに置くと、達成度を上げながら継続しやすくなります。';
  } else if (lowestScore === consistency) {
    comment =
      '記録のない日が続くと継続度が下がります。短時間でも記録を1件残す日を増やしてください。';
  } else if (lowestScore === realism) {
    comment =
      '計画と記録の時間差が大きめです。次回は開始時刻か終了時刻を一段階だけ現実寄りに調整しましょう。';
  }
