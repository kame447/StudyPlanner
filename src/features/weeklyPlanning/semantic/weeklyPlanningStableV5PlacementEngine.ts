...
interface PlacementRuntimeContext {
...
function findWorkItemSlot(params: {
  context: PlacementRuntimeContext;
  item: GenericPlanningWorkItem;
  dates: string[];
  duration: number;
...
function scheduleWorkItem(params: {
  context: PlacementRuntimeContext;
  item: GenericPlanningWorkItem;
  ...
  for (let chunkIndex = 0; chunkIndex < chunks.length; chunkIndex += 1) {
    ...
    if (isVocabulary) {
      ...
      if (failedReviewId) {
        return { candidates: itemCandidates, failedWorkItemId: failedReviewId };
      }
    }
  }
...
export function scheduleWeeklyPlanningStableV5Preview(params: {
  ...
  const context: PlacementRuntimeContext = {
    input: params.input,
    graph: params.graph,
    dates,
  ...
  for (const item of params.input.movableWorkItems) {
    const scheduled = scheduleWorkItem({
      context,
      item,
