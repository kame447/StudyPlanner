...
describe('Stable V5 semantic meaning-rule inventory', () => {
  ...
  it('keeps historically load-bearing semantic distinctions explicit after prompt compaction audits', () => {
    ...
    expect(instructionFor('temporal_kind_and_strength')).toContain('This strength choice applies to task timing constraints. A free-time window the user states as available is an available declaration with hard strength unless the user explicitly calls that window optional or uncertain.');
