...
const SEMANTIC_DELTA_CONTEXT_INSTRUCTION_V5 = [
  'Context is not output. publicStateSummary and recentConversation are context only. Emit only current userText changes and permitted descriptive supplementalContext facts; each sourceText must be supported by current userText or, for those descriptive image facts only, supplementalContext. Do not replay unchanged facts. New nested facts on existing entities need only a minimal shell with exact existingPublicId.',
