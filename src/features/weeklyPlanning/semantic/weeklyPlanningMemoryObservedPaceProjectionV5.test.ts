...
function graph(explicit = false): WeeklyPlanningGenericSchedulerGraphView {
  ...
  return {
    revision: 1,
    planningWindows: [],
    tasks: [{ id: 'task', category: 'study', title: '英単語', source, createdRevision: 1 }],
    components: [],
    workloads: [{
      id: 'workload',
      taskId: 'task',
...
describe('observed memory pace scheduler projection', () => {
  it('uses persisted pace only after Luna classified the workload as memorization', () => {
    ...
    const result = projectWeeklyPlanningMemoryObservedPaceV5({
      ownerId,
      graph: graph(),
      document: document('memorization_retrieval'),
  ...
  it('does not infer memorization from the word unit alone', () => {
    ...
    const result = projectWeeklyPlanningMemoryObservedPaceV5({
      ownerId,
      graph: graph(),
      document: document('problem_solving'),
      localToFactId: { 'task-local': 'task', 'workload-local': 'workload' },
      previousRecords: [],
    });
