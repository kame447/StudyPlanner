...
describe('Stable V5 durable goal-event partial date normalization', () => {
  it('resolves whole months and month parts without inventing an exact event day', () => {
    expect(resolveWeeklyPlanningUserContextPartialDateV5('year:2027;month:01')).toEqual({
      start: '2027-01-01',
      end: '2027-01-31',
    });
    expect(resolveWeeklyPlanningUserContextPartialDateV5('year:2027;month:02;part:late')).toEqual({
      start: '2027-02-21',
      end: '2027-02-28',
    });
    expect(resolveWeeklyPlanningUserContextPartialDateV5('year:2028;month:02;part:late')).toEqual({
      start: '2028-02-21',
      end: '2028-02-29',
    });
  });
  ...
  it('canonicalizes provider-interpreted goal-event periods to ISO date ranges', () => {
    const result = normalizeWeeklyPlanningUserContextPartialDatesV5(
      documentWithDate('year:2027;month:02;part:late'),
    );
  ...
  it('leaves invalid or non-goal-event expressions untouched', () => {
    const invalid = normalizeWeeklyPlanningUserContextPartialDatesV5(
      documentWithDate('year:2027;month:13'),
    );
    expect(invalid.repairs).toEqual([]);
    expect(invalid.rawResponse).toBe(documentWithDate('year:2027;month:13'));

    const concern = normalizeWeeklyPlanningUserContextPartialDatesV5(
      documentWithDate('year:2027;month:01', 'concern'),
    );
