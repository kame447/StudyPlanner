export function createWeeklyPlanningSemanticMeaningPolicyV5(): string {
  return [
    ...
    'Keep amount declared when target/remaining/completed is unclear. When one vague progress statement makes both completed and remaining unknown, emit only the root progress uncertainty; do not duplicate a derived remaining/completed uncertainty that depends only on the same unknown value. Distinguish workload from duration; use per-unit effort only when stated.',
