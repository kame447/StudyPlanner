export function createWeeklyPlanningSemanticMeaningPolicyV5(): string {
  return [
    "Interpret the user's planning meaning into semantic facts. Use only supported evidence; schema and deterministic validators own canonical wire shape.",
    ...
    'Temporal facts say when work may or must happen. Keep mandatory/unavailable/deadline constraints hard, preferences soft, and unresolved strength unknown. An event date is a goal event unless the user also states completion-by meaning.',
    'Create concern only from explicit difficulty, worry, weakness, low confidence, lag, or motivation evidence; never infer concern or priority from workload size, frequency, or duration.',
