...
export const WEEKLY_PLANNING_SEMANTIC_MEANING_RULES_V5 = [
  ...
  },
  {
    ...
    instruction: 'scope_total requires bounded total; target requires plan amount and is never derived. Qualitative scope belongs in task/components, not amount=1 custom workload. Approved material chapter/section structure resolves work_breakdown without exact names/counts; don’t re-ask. Otherwise use work_breakdown only for genuine structural ambiguity; never invent quantity or total duration. All-remaining omits target; code derives remaining. completed/remaining are progress; scores/grades/accuracy/rank are not unless explicit task/material completion. Scheduled time is target minute/hour workload; don’t ask content progress. Align perOccurrence/recurrence. total_duration/duration_per_unit are workload cost; session_duration one session.',
  },
  {
