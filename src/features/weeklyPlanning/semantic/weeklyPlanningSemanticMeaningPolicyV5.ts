...
export const WEEKLY_PLANNING_SEMANTIC_MEANING_RULES_V5 = [
  ...
  },
  {
    ...
    instruction: "scope_total needs bounded total; target is plan amount, never derived. Qualitative scope goes in task/components, not amount=1 custom workload. Approved material chapter/section structure resolves work_breakdown without exact names/counts; don't re-ask. Otherwise work_breakdown only for genuine structural ambiguity; never invent quantity/total duration. All-remaining omits target; code derives remaining. completed/remaining are progress; scores/grades/accuracy/rank are not unless explicit task/material completion. Scheduled time is target minute/hour workload; do not ask content progress. Align perOccurrence/recurrence. Separate workload cost: total_duration/duration_per_unit; session_duration only for one session.",
  },
  {
