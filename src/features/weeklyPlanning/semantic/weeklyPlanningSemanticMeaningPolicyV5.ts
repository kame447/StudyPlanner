...
export const WEEKLY_PLANNING_SEMANTIC_MEANING_RULES_V5 = [
  ...
  },
  {
    ...
    instruction: 'Use quantityRole=scope_total only for an explicitly bounded total task size, not merely the amount planned now. target is planned work, remaining unfinished work, completed current progress. Omitted workload or progress is absence, not semantic uncertainty; the application asks for missing information. Emit uncertainty only when current wording expresses workload/progress meaning that is genuinely ambiguous. Explicit completion percentages use quantityRole=completed, unitCode=custom, unitLabel="%", amount 0..100; do not invent page/problem/slide counts. Distinguish workload from duration; per-unit effort only when stated.',
  },
  {
