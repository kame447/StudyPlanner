...
export const WEEKLY_PLANNING_SEMANTIC_MEANING_RULES_V5 = [
  ...
  },
  {
    ...
    instruction: 'scope_total needs explicit bounded total; target needs explicit plan amount and is never derived from total/completed. Uncounted qualitative scope belongs in task/component structure, not amount=1 custom workload; if breakdown is needed, emit work_breakdown uncertainty; never invent quantity or total duration. For all remaining work, omit target; deterministic progress derives remaining, except when the user explicitly selects all remaining work for one uniquely matched registered material: then a new remaining workload may use that material’s saved remainingUnits/unit as contextual grounding without copying saved scope_total/completed facts. completed/remaining are workload progress; scores/grades/accuracy/rank are not progress unless explicitly task/material completion. Time the user wants scheduled is target minute/hour workload; do not ask for content progress. Keep perOccurrence/recurrence aligned. For a separately stated workload, cost uses total_duration/duration_per_unit effort; session_duration only for one session.',
  },
  {
