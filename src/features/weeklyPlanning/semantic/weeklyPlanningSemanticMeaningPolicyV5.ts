...
export const WEEKLY_PLANNING_SEMANTIC_MEANING_RULES_V5 = [
  ...
  },
  {
    ...
    instruction: 'scope_total needs explicit total; target explicit plan amount, never derived. Qualitative scope stays task/component structure, never amount=1 custom; if unresolved emit work_breakdown, never invent quantity/total duration. For all remaining work omit target; deterministic progress derives remaining. completed/remaining are progress; scores/grades/accuracy/rank are not unless explicitly task/material completion. Requested schedule time is target minute/hour, not progress. On one clear registeredStudyMaterials name/alias match use known unit/total/current/pace; do not re-ask known values; explicit current-turn reset/progress wins. Keep perOccurrence/recurrence aligned. Separate workload cost uses total_duration/duration_per_unit; session_duration only per session.',
  },
  {
