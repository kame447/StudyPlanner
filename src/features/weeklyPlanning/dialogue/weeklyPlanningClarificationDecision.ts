...
import { createMissingQuestionPlan } from './weeklyPlanningDialogueManagerCore';
import type { WeeklyPlanningDialogueDecision as CoreDecision } from './weeklyPlanningDialogueManagerCore';

export type WeeklyPlanningDialogueDecision = Omit<CoreDecision, 'clarification'> & {
  clarification?: {
    explanation: string;
    targetSlot?: string;
    intent?: string;
...
function resolveTarget(params: {
  ...
  if (params.target === 'referenced_question') return params.previousQuestionContext?.targetSlot;
...
function explanationFor(targetSlot: string | undefined): string {
  if (!targetSlot) return GENERIC_EXPLANATION;
  return termExplanationForSlot(targetSlot)
    ?? CONTEXT_EXPLANATIONS[targetSlot]
    ?? GENERIC_EXPLANATION;
}
...
export function createWeeklyPlanningClarificationDecision(params: {
  ...
  const questionPlan = targetSlot
    ...
    : [];
  const previous = params.previousQuestionContext;
  const intent = targetSlot && previous?.targetSlot === targetSlot
    ? previous.intent
  ...
  return {
    ...
    questionPlan: questionPlan.length ? questionPlan : undefined,
    clarification: {
