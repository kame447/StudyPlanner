...
function normalizeClarificationText(text: string): string {
  return normalizeIntakeText(text).replace(/\s+/g, '').replace(/[?？!！。、]/g, '').trim();
}
...
export function parseRequestClarificationCommand(
  ...
  const contextualFollowUp = context.activeQuestionSource === 'rendered'
    && Boolean(context.hasActiveQuestion)
