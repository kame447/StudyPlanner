...
test('common material aliases resolve from the curated seed and keep the catalog link after saving', async ({ page }) => {
  ...
  const materialName = 'TOEIC L&R TEST 出る単特急 金のフレーズ';
  const candidate = sheet.locator('.material-metadata-result').filter({
    hasText: materialName,
  });
  await expect(candidate).toBeVisible();
  await candidate.click();
  await expect(sheet.getByLabel('教材名', { exact: true })).toHaveValue(materialName);
  ...
  await expect(page.locator('.bookshelf-view')).toContainText(materialName);

  await page.getByRole('button', { name: `${materialName}のメニュー` }).first().click();
  await page.getByRole('button', { name: /教材情報・進捗を編集/ }).click();

  const editSheet = page.locator(
    '.bookshelf-view > .modal-overlay:has(> .bookshelf-modal .bookshelf-material-edit-grid)',
  );
  await expect(editSheet).toBeVisible();
  await expect(editSheet).toContainText('教材DBに紐付け済み');
});
