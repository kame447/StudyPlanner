...
Do not create `docs/ai/tasks`, agent-specific queues, retry documents, or a second roadmap for the same execution order. Tool-specific operational knowledge belongs in the runbook only when it is durable and repository-wide; one-off incidents remain in their Issue/PR/Actions evidence.