...
この文書はStable V5の実装到達点を記録する。PR #77でStable V5 runtime path、PR #79でbrowser session persistenceとGraph commit atomicityをmainへ導入済みである。default runtimeはlegacyであり、全ユーザーcutoverではない。
...
AIはユーザー発話の意味構造化だけを担当する。schema validation、fact ID、revision、short-answer binding、readiness、question selection、planning horizon、existing plan / timetable制約、availability、placement、preview、approval、saveはアプリ側が決定する。
...
conversation IDを復元した場合、controllerはPlanningStateに保存されたmessage IDから最大turn sequenceを復元する。再マウント後に`turn:1`、`request:1`、message IDを再発行しない。
...
これは同一browser内の保存であり、server repositoryまたはcross-device persistenceではない。旧PlanningStateからStable V5 Graphへのdeterministic migration decoderは未実装である。
...
2026-07-24の実トレースから、conversation persistence後にtrace runtimeとcontroller counterが初期化され、同じ`logicalConversationId`が複数trace sessionへ分裂する不具合を確認した。本branchで次を修正した。

- metadata-only trace cursor
- 同一conversationのtrace session ID継続
...
cursorへconversation本文、assistant本文、semantic document、Fact Graphを保存しない。
...
本branchのtest、typecheck、buildはCIまたはlocal checkoutの実行結果を取得するまで成功確認済みと表記しない。
