...
- 作成日: 2026-07-02
- 実コード基準: `note_progress_boundary` command 導入後(progressHint の command 化実装済み・承認待ち時点)
...
順序の骨格: **R1(境界完成)→ R2/R3(入力理解の拡大)→ R4(質問計画)→ R5(プロファイル)→ R6(進捗記録)→ R7(再計画)→ R8(個別最適化)**。
...
6. **個別最適化(R8)は最後。** 配置スコア・6等分・休憩などの品質改善は、入力理解と進捗データが揃うほど効果が出る。先にやると、後段の型変更のたびに scheduler を触り直すことになる。二系統 scheduler の統合は最も高リスクで、独立した設計文書なしに着手しない。
...
現在オープンなタスク: `docs/ai/tasks/20260702-weekly-planning-progress-hint-command.md`(R1 相当・実装済み・承認待ち)。承認後、これが closed/ への最初の移動対象になる。
