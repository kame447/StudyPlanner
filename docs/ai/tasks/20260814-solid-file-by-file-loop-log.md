...
| 38 | `src/components/DailyMaterialShelf.tsx` | adversarial audit found a behavior regression: sorting grouped materials before choosing missing-subject fallback metadata could change the displayed subject name/color. Reused `buildSubjectsWithMaterialFallback` so fallback identity is derived from source order before card sorting. | Added `DailyMaterialShelf.test.tsx` for conflicting legacy fallback metadata + sorted card order. PR #129 CI/Browser Regression execute the branch tests; do not mark verification green until those runs pass. | done |
