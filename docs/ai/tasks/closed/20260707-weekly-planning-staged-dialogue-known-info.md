...
> **完了記録(2026-07-07・コミット `8d695d2`)**: dialogueManager に questionPlan(1〜2論点への絞り込み・targetSlot・intent・kind)を導入し、missing 全列挙ではなく優先度順の質問計画を持たせた。これが question-rendering-separation(R2-D)の RenderInput.nextQuestions の入力契約になる。weeklyPlanning テスト green / build 成功。完了条件は Phase 2 候補(a)「1ターンの提示論点の絞り込み」で充足(候補(a)/(b) は or 関係で全部は不要)。既知カレンダー予定の intake 注入(差分提示・候補(b))と分割許可の確認対話(候補(c))は本タスクでは未着手で、後続候補として roadmap に記録。
