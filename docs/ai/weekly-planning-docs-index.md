...
| document | role |
| --- | --- |
| [weekly-planning-current-contract-status.md](weekly-planning-current-contract-status.md) | active文書間のstatus、queue、decision pendingの読み方 |
...
| [weekly-planning-roadmap.md](strategy/weekly-planning-roadmap.md) | current implementation status、decision gates、current queue |
...
active文書間でstatus、queue、未決定contractが競合する場合は、`weekly-planning-current-contract-status.md`とroadmap Decision gatesを先に確認する。これらはproduct decisionを代行せず、決定前の誤実装を防止するためのoverlayである。

上記文書と、`docs/ai/tasks/`直下の未完了task以外をcurrent instructionとして扱わない。

## Current queue

現在のroot taskは次の6件である。
...
3. [20260716-weekly-planning-trace-privacy-and-lifecycle.md](tasks/20260716-weekly-planning-trace-privacy-and-lifecycle.md)
   - product decision後にopt-in、raw content、TTL、account deletion、admin accessを同期する。
