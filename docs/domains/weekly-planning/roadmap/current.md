...
The evaluation foundation from #333 (tuning / fixed-holdout execution, same-case Jev/Luna comparison, Luna responsibility inventory) was merged through PR #336. The active work is now replacement units under #305: each unit runs Jev first in a temporary test environment through the production dispatch, verifies the real Luna fallback for low confidence, abstention, unsupported cases and provider faults, calibrates on tuning only, evaluates a sealed holdout once, compares with Luna alone on the same cases, and re-runs the affected #335 regressions. Unit records live in `../work/20260927-issue305-jev-first-*.md`; branch, PR and orchestration checkpoints live in #305.
